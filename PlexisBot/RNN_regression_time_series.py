...
class recurrent_neural_net:
    ...
    def __data_prep(self):
        ...
        for count, row in enumerate(range
                (num_val_samples + num_train_samples, num_val_samples + num_train_samples + 10)):
            print('Predicted Value: ', predictions[count][0] * std[3] + mean[3],
                  'Actual Value: ', self.features[row].split(',')[4])
        return train_run

    def performance(self):
        """retrieves the metrics from the training or test run"""
